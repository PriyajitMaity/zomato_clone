...
const GenerateRestaurant = ({ restaurant }) => {
  ...
  return (
    <Link to={`${restaurant.cardAction?.clickUrl}/${restaurant.info?.resId}`} className="generate-restaurant" onMouseEnter={startRotated} onMouseLeave={stopRotated} >
      <div className="restaurant-image">
        {restaurant.isPromoted && <div className="promoted">Promoted</div>}
        {restaurant.bulkOffers.length > 0 && (
          <div className="offer">{restaurant.bulkOffers[0]?.text}</div>
        )}
        ...
        <div className="left-side">
          <div className="rotation-text" style={{transform: rotation && `translate(-16.5rem, 0)`}}>
            <div className="rotation-image">
              <GenImage url={restaurant.bottomContainers[0]?.image?.url} alt="" />
            </div>
            <span>{restaurant.bottomContainers[0]?.text}</span>
          </div>
          <div className="rotation-text" style={{ transform: rotation && `translate(-16.5rem, 0)` }}>
            <div className="rotation-image">
              <GenImage url={restaurant.bottomContainers[1]?.image?.url} alt={""} />
            </div>
